...
int main(int argc, char ** argv)
{
  ...
  // 現在の状態モニタのためのシングルスレッドエグゼキュータを作成
  ...
  move_group_arm.setMaxVelocityScalingFactor(1.0);  // 速度スケーリングファクターを設定 (0.0 ~ 1.0)
  move_group_arm.setMaxAccelerationScalingFactor(1.0);  // 加速度スケーリングファクターを設定 (0.0 ~ 1.0)

  MoveGroupInterface move_group_gripper(move_group_gripper_node, "gripper");
  move_group_gripper.setMaxVelocityScalingFactor(1.0);  // 速度スケーリングファクターを設定 (0.0 ~ 1.0)
  move_group_gripper.setMaxAccelerationScalingFactor(1.0);  // 加速度スケーリングファクターを設定 (0.0 ~ 1.0)
